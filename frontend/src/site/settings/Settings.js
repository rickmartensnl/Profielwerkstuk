                ...
                <div className="container mx-auto px-5">
                    ...
                    <form onSubmit={this.handleSubmit} className={`${this.state.dyslexia ? 'dyslexia-font' : ''}`}>
                        <Switch.Group as="div" className="flex items-center space-x-4 mt-2">
                            ...
                            <Switch as="button" checked={this.state.darkMode} onChange={this.setDarkMode} className={`${this.state.darkMode ? 'bg-green-400' : 'bg-gray-200'} mr-5 relative inline-flex flex-shrink-0 h-6 w-12 translation-colors duration-200 ease-in-out border-2 border-transparent rounded-full cursor-pointer focus:outline-none focus:shadow-outline`}>
                                <span className={`${this.state.darkMode ? 'translate-x-6' : 'translate-x-0'} inline-block w-5 h-5 translation duration-200 ease-in-out transform bg-white rounded-full`} />
                            </Switch>
                        </Switch.Group>
                        <Switch.Group as="div" className="flex items-center space-x-4 mt-2">
                            <Switch.Label>Dyslexie modus</Switch.Label>
                            <Switch as="button" checked={this.state.dyslexia} onChange={this.setDyslexiaMode} className={`${this.state.dyslexia ? 'bg-green-400' : 'bg-gray-200'} mr-5 relative inline-flex flex-shrink-0 h-6 w-12 translation-colors duration-200 ease-in-out border-2 border-transparent rounded-full cursor-pointer focus:outline-none focus:shadow-outline`}>
                                <span className={`${this.state.dyslexia ? 'translate-x-6' : 'translate-x-0'} inline-block w-5 h-5 translation duration-200 ease-in-out transform bg-white rounded-full`} />
                            </Switch>
                        </Switch.Group>
                        <button type="submit" className="whitespace-nowrap mt-10 px-4 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-blue-500 hover:bg-blue-600">Opslaan - WERKT NIET</button>
                    </form>
                </div>
            </div>
        );

...
export class ParagraphsChild extends React.Component {
    ...
    constructor(props) {
        ...
        this.paragraph = props.data.paragraph;

        this.authMiddleware = this.props.data.authMiddleware;
    }

    render() {
        let verified = (this.paragraph.flags & 0x1) === 0x1;
                ...
                <div className="content-center">
                    <h3 className={`font-bold text-lg inline-block ${this.props.data.dyslexia ? 'dyslexia-font' : ''}`}>
                        ...
                        <div className={`has-tooltip inline-block fill-current align-middle ml-2 -mt-1.5 ${verified ? '' : 'hidden'}`}>
                            <span className="tooltip rounded shadow-lg p-1 bg-gray-100 dark:bg-dark-tertiary -mt-10 text-base font-normal">Verified Paragraph</span>
                    ...
                    <Link to={`/subjects/${this.subject.uuid.replaceAll('-', '')}/${this.chapter.uuid.replaceAll('-', '')}/${this.paragraph.uuid.replaceAll('-', '')}/play`} className="p-2 text-right inline-block float-right bg-blue-500 rounded-lg">
                        Ga verder met leren!
                    </Link>
                </div>
